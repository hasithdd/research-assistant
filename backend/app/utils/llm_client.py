...
def _truncate_prompt(prompt: str, max_tokens: int = 400) -> str:
    """Truncate prompt to fit within an approximate token budget."""
...
def _call_vllm(
    messages: List[Dict[str, str]], max_tokens: int = 250, temperature: float = 0.0
) -> Optional[str]:
    ...
    try:
        ...
        for msg in messages:
            ...
            if msg["role"] == "user":
                truncated_msg["content"] = _truncate_prompt(
                    msg["content"], max_tokens=700
                )
...
def call_llm(
    ...
    if (
        ...
        and not settings.OPENAI_API_KEY.startswith("#")
    ):
        start_time = time.time()
        try:
            import openai
    ...
    logger.error(
        "All LLM providers failed, returning safe fallback text instead of prompt"
    )
